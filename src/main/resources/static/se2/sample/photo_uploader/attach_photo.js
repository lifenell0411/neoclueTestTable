    ...
    function callFileUploader (){
        // ✅ 1단계: 디버깅 로그 추가
        // HTML의 input 요소를 제대로 찾았는지 확인합니다.
        var elUploadInput = jindo.$("uploadInputBox");
        console.log("Jindo가 찾은 파일 입력창:", elUploadInput);

        // 만약 여기서 null이 출력된다면, photo_uploader.html의 <input> 태그 id를 확인하세요!
        if (!elUploadInput) {
            alert("오류: 업로드 입력창(uploadInputBox)을 찾을 수 없습니다. HTML의 id를 확인해주세요.");
            return;
        }

        oFileUploader = new jindo.FileUploader(elUploadInput,{
            // ✅ 2단계: URL 수정
            // 기존 PHP 경로 대신, 우리가 HTML에서 전달받은 g_sUploadURL 변수를 사용합니다.
            sUrl : g_sUploadURL,
            sCallback : '', // iframe 방식에서는 컨트롤러 응답이 바로 콜백 역할을 하므로 비워둡니다.
            ...
            select : function(oCustomEvent) {
                console.log("파일 선택 이벤트 발생!", oCustomEvent);
                if(oCustomEvent.bAllowed === true){
                    goStartMode();
                    console.log("버튼 활성화 (goStartMode) 호출됨");
                }else{
                    goReadyMode();
                    oFileUploader.reset();
                    console.log("지원하지 않는 파일 형식으로 버튼 비활성화 (goReadyMode) 호출됨");
                }
            },
            success : function(oCustomEvent) {
                // iframe 방식에서는 컨트롤러가 반환하는 <script>가 직접 부모창의 함수를 호출하므로,
                // 이 success 핸들러가 직접적으로 사용되지 않을 수 있습니다.
                // 하지만 만약을 위해 남겨둡니다.
                console.log("업로드 성공!", oCustomEvent);
                var aResult = [];
            ...
            error : function(oCustomEvent) {
                console.error("업로드 실패!", oCustomEvent);
                alert(oCustomEvent.htResult.errstr || "파일 업로드 중 오류가 발생했습니다.");
            }
        });
 	}
